...
export class SubstrateActions implements AccountActionHandler {
  ...
  async withdrawAllPayouts(stakingAddress: string) {
    if (this.isCryptoWasmReady) {
      ...
      this.signer!.signTransaction({
        ...
        transactionData:
          this.networkAPI?.tx.staking.withdrawUnbonded(slashingSpans),
      })
        .then((signedTx) => {
          (signedTx as SubmittableExtrinsic<"promise">)
            .send(({ events = [], status }) => {
              if (status.isInBlock) {
                events.forEach(
  ...
  async unstakeFunds(amountToUnstakeWhole: string) {
    if (this.isCryptoWasmReady) {
      ...
      let stakingStatus = await this.networkAPI?.query.staking.ledger(
        this.address
      );
      if (stakingStatus != undefined && stakingStatus.toJSON() != null) {
        ...
        console.error(
          "Failed to get the current staked amount, which is necessary info for unbonding."
        );
        return;
      }

      let leftoverStake = new BigNumber(bondedAmount).minus(
        amountToUnstakeWhole
      );
  ...
  async stakeFunds(bondData: SubstrateBondData) {
    if (this.isCryptoWasmReady) {
      let kp = this.keyring?.addFromUri(
        this.seed + bondData.stakingAddressDerivationPath
      );
      if (kp == undefined) {
        console.error(
          "No signature keypair could be generated for staking address!"
        );
        return;
      }
      ...
      let minNominationAmount =
        await this.networkAPI?.query.staking.minNominatorBond();
      if (minNominationAmount != undefined) {
        minBond = new BigNumber(
          (minNominationAmount.toHuman() as string).replaceAll(",", "")
        );
      } else {
        postMessage({
      ...
      let ledger = await this.networkAPI?.query.staking.ledger(
        bondData.controllerAddress
      );
      if (ledger != undefined) {
      ...
      if (
        stakedBalance.comparedTo(minBond) == 1 ||
        stakedBalance.comparedTo(minBond) == 0 ||
        isStashBonded == true
      ) {
        this.signer!.signTransactionBy(bondData.stakingAddressDerivationPath, {
          transactionFormat: TransactionFormats.SubstrateExtrinsic,
          transactionData: this.networkAPI?.tx.staking.bondExtra(
            bondData.bondAmountWhole
          ),
        })
          .then((signedTx) => {
            (signedTx as SubmittableExtrinsic<"promise">)
        ...
        this.networkAPI?.tx.staking
          .bond(bondData.controllerAddress, bondData.bondAmountWhole, "Stash")
          .signAndSend(kp!, ({ events = [], status }) => {
            if (status.isInBlock) {
              events.forEach(({ event: { data, method, section }, phase }) => {
                if (method == "ExtrinsicSuccess") {
                  postMessage({
  ...
  async generateAddress(derivationPath: string, accountIndex?: number) {
    if (this.isCryptoWasmReady) {
      ...
      postMessage(
        new MethodResponse(VultureMessage.GET_ADDRESS_FROM_URI, {
          success: true,
          address: kp.address,
  ...
  async estimateTxFee(recipent: string, amount: string, token?: AbstractToken) {
    if (this.isCryptoWasmReady) {
      if (token != null) {
        ...
        contract.tx
          .transfer({ value: 0, gasLimit: -1 }, recipent, amount)
          .paymentInfo(this.keypair!)
          .then((info: any) => {
            postMessage({
        ...
        this.networkAPI!.tx.balances.transfer(recipent, amount)
          .paymentInfo(this.keypair!)
          .then((info: any) => {
            postMessage({
              method: VultureMessage.ESTIMATE_TX_FEE,
  ...
  async updateAccountsToNetwork(accounts: AccountData[], network: Network) {
    if (this.isCryptoWasmReady) {
      ...
      for (let i = 0; i < accounts.length; i++) {
        ...
        let kp = this.keyring!.addFromUri(
          this.seed + "//" + updatedAccounts[i].accountIndex
        );
        updatedAccounts[i].address = kp.address;
      }
      postMessage(
        new MethodResponse(VultureMessage.UPDATE_ACCOUNTS_TO_NETWORK, {
          success: true,
  ...
  async transferAssets(
    ...
    amount: string,
    token?: AbstractToken,
    from?: { address: string; derivationPath: string }
  ) {
    if (this.isCryptoWasmReady) {
