...
export class SubstrateInfo implements AccountInfoHandler {
      ...
      .then((ready) => {
        ...
        this.wsProvider.isReady.then(() => {
          ApiPromise.create({ provider: this.wsProvider }).then((api) => {
            this.networkAPI = api;
            postMessage(
              new MethodResponse(VultureMessage.SET_CURRENT_WALLET, {
                success: true,
  ...
  async getValidatorInfo() {
    if (this.isCryptoReady) {
      ...
      let validators = await this.networkAPI?.query.staking.erasRewardPoints(
        Number(currentEra) - 1
      );
      if (validators != undefined && validators.toJSON() != null) {
        let currentValidators: string[] = Object.keys(
          (validators.toJSON() as any).individual
        );
        ...
        for (let i = 0; i < currentValidators.length; i++) {
          ...
          let validatorPrefs =
            await this.networkAPI?.query.staking.erasValidatorPrefs(
              currentEra,
              currentValidators[i]
            );
          if (validatorPrefs != undefined && validatorPrefs.toJSON() != null) {
            validatorComission = String(
              (validatorPrefs.toJSON() as any).commission
            );
          } else {
            console.error(
              "Failed to get validator info for: " + currentValidators[i]
            );
          }
          ...
          try {
            let validatorIdentity =
              await this.networkAPI?.query.identity.identityOf(
                currentValidators[i]
              );
            if (
              validatorIdentity != undefined &&
              validatorIdentity.toJSON() != null
            ) {
              let res = validatorIdentity.toHuman(true) as any;
              identity.twitter =
                res.info.twitter.Raw == null ? undefined : res.info.twitter.Raw;
              identity.name =
                res.info.display.Raw == null ? undefined : res.info.display.Raw;
              identity.email =
                res.info.email.Raw == null ? undefined : res.info.email.Raw;
              identity.webURI =
                res.info.web.Raw == null ? undefined : res.info.web.Raw;
            }
          } catch (error) {
  ...
  async getStakingInfo(address: string, stakingAddress: string) {
    if (this.isCryptoReady) {
      ...
      if (bonded != undefined && bonded.toHuman() == null) {
        ...
        success = false;
        console.error(
          "Staking account is bonded to an incorrect controller address!"
        );
      }

      let minNominationAmount =
        await this.networkAPI?.query.staking.minNominatorBond();
      ...
      let nominationData = await this.networkAPI?.query.staking.nominators(
        stakingAddress
      );
      if (nominationData != undefined && nominationData.toJSON() != null) {
        if ((nominationData.toJSON() as any).targets.length >= 1) {
          stakingInfo.nominationAddress = (
            nominationData.toJSON() as any
          ).targets[0];
        }
      } else {
        console.info(
          "Failed to get staking address nominations. Likely due to the staking address not having a nomination."
        );
      }
      ...
      if (account != undefined) {
        ...
        let miscFrozen = new BigNumber(
          (account.toJSON() as any).data.miscFrozen
        );

        let liquidAmount = free.minus(BigNumber.max(feeFrozen, miscFrozen));

        stakingInfo.frozenBalance = BigNumber.max(
          feeFrozen,
          miscFrozen
        ).toString();
  ...
  async getTokenMetadata(
    ...
    switch (tokenType) {
      case TokenTypes.ERC721: {
        let erc721contract = new ContractPromise(
          this.networkAPI!,
          erc721Abi,
          tokenAddress
        );
        await getERC721Metadata(
          tokenAddress,
          erc721contract,
          this.address,
          tokenId!
      ...
      default: {
        console.log(
          "Token Type: " +
            tokenType +
            " doesn't contain a metadata method! (At least one that is supported by vulture)"
        );
        break;
      }
    }
  ...
  async subscribeToAddressEvents(address?: string) {
    if (this.isCryptoReady) {
      const unsub = await this.networkAPI?.query.system.account(
  ...
  async getBalanceOfToken(tokenAddress: string, tokenType: string) {
    if (this.isCryptoReady) {
      switch (tokenType) {
        case "ERC20": {
          let erc20contract = new ContractPromise(
            this.networkAPI!,
            erc20Abi,
            tokenAddress
          );
        ...
        default: {
          console.error(
            "Tried getting balance of token, but the tokenType is invalid!"
          );
          postMessage({
    ...
    if (this.isCryptoReady) {
      try {
        switch (tokenType) {
          ...
          default: {
            console.error(
              "TokenType: " + tokenType + " is invalid for this method!"
            );
          }
        }
      } catch (error) {
        console.log(error);
        postMessage(
          new MethodResponse(VultureMessage.GET_TOKEN_DATA, {
            tokenData: null,
